 ...
 */
@Service(value = "all-index-new")
class AllIndexServiceImplNew implements ScanService {

    private static final Logger logger = Logger.getLogger(AllIndexServiceImplNew.class)
    ...
    AllIndexFrMongoResultHandlerNew resultHandler = new AllIndexFrMongoResultHandlerNew()
    @Autowired
    ...
    public void updateMarcoIndexToMgDB() {
        ...
        for (dds in dataSrcs) {
            ...
            try {
                ...
                String rowSql = StringUtils.ClobToString(dds.SQL_CLAUSE) + " where UPD_TIME >= TO_DATE('" + DateUtils.date2String(oldUpdTime, DateUtils.FORMAT_DATETIME) + "', " + "'YYYY-MM-DD HH24:MI:SS')" + " order by UPD_TIME asc";
                executeHandler(dds.db_name, dsRow, rowSql, null, dsName);
        ...
        public void run() {
            try {
                ...
                if (operType == OPER_TYPE_SAVE) {
                    ...
                    logger.error("指标数据源【" + wtExeInsLst.get(0).DS_ENG_NAME + "】增量更新失败：" + e.toString());
    ...
    private void execAllIdxFormMongo(List<Map<String, Object>> wtExeInsLst, String operType, Map<String, Date> getScanTime) {
        ...
        //静态分段
            ...
            AllIndexFrMongoResultHandlerNew fullParamRowFromMongo = new AllIndexFrMongoResultHandlerNew(indexObjOne, result,
                    paramMap, wtExeInsLstTmp, propsMap, allIndexTemplate, mongoTemplatePool)
            if (operType == OPER_TYPE_SAVE) {
                String sql = indexObjOne.DS_PARAM_FULL
                executeHandler(indexObjOne.db_name, fullParamRowFromMongo, sql, null, indexObjOne.DS_ENG_NAME);
            ...
            if (operType == OPER_TYPE_SAVE) {
                ...
                String endDateTime = DateUtils.date2String(getScanTime.get(END_TIME), DateUtils.FORMAT_DATETIME)
                // 获取更新参数的sql
                String sql = StringUtils.ClobToString(indexObjOne.get("UPD_PARAM")).replace("\${BGN_TIME}", beginDateTime).replace("\${END_TIME}", endDateTime)
                executeHandler(indexObjOne.db_name, fullParamRowFromMongo, sql, null, indexObjOne.DS_ENG_NAME);
...
//                List<Map<String, Object>> piecewiseList = rawSQLService.queryRawSql(updParamsql, sqlParam);
            ...
            for (int i = 0; i < result.length; i++) {
                ...
                joinSql.append(" A.").append(result[i].getName()).append(" = B.").append(result[i].getName());
            }
            if (operType == OPER_TYPE_SAVE) {
...
//                saveIdxFromDb(wtExeInsLstTmp, props, dsSql, null)
                ...
                sql.append("SELECT A.* FROM (")
                        .append(com.bigdata.datacenter.datasync.utils.StringUtils.ClobToString(indexMap.SQL_CLAUSE))
                        .append(") A INNER JOIN (")
                        .append(com.bigdata.datacenter.datasync.utils.StringUtils.ClobToString(indexMap.INIT_SQL))
                        .append(") B ON ");
                saveIdxFromDb(wtExeInsLstTmp, props, sql.append(joinSql).toString(), null)
...
//                }
                ...
                dataUpdSql.append("SELECT A.* FROM (")
                        ...
                        .append(updParamsql)
                        .append(") B ON ");
                saveIdxFromDb(wtExeInsLstTmp, props, dataUpdSql.append(joinSql).toString(), sqlParam);
            }
        }
        // 结束操作更新状态
    ...
    private void saveIdxFromDb(List<Map<String, Object>> wtExeInsLst, List<Map<String, Object>> props, String sql, Map<String, Object> sqlParams) {
        ...
        Collections.sort(Arrays.asList(result))
        Map<String, Map<String, Object>> propsMap = getProsMap(props)
        AllIndexFrDBResultHandler dsRow = new AllIndexFrDBResultHandler(result, propsMap, wtExeInsLst, allIndexTemplate, mailService)
        //判断是否有sql参数
        if (sqlParams != null) {
    ...
    def getScanTime() {
        ...
        if (lastScanTimeStr == null || lastScanTimeStr == "") {
            ...
            endDateTime = DateUtils.date2String(date, DateUtils.FORMAT_DATETIME);
            beginDateTime = PropertiesUtil.getProperty(PropertiesUtil.LAST_SCAN_TIME_FILE,PropertiesUtil.LAST_SCAN_TIME);
        }
    ...
    private void executeHandler(String db_name, ResultHandler resultHandler, String sql, Map params, String ds_name) {
        ...
        try {
            ...
            logger.debug("executeHandler(): 保存数据源【" + db_name + "】的数据，获取数据源的数据连接。----" + dbName);
